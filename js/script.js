...
const PRESETS = [
  ...
  },
  {
    ...
    thumb:
      "https://images.unsplash.com/photo-1514565131-fce0801e5785?w=300&q=60",
  ...
  try {
    ...
    if (saved) {
      ...
      state.overlayOpacity = Math.min(
        ...
        Math.max(
          0,
          parseInt(data.overlayOpacity) || DEFAULT_STATE.overlayOpacity
        )
      );
      state.overlayColor = data.overlayColor || DEFAULT_STATE.overlayColor;
      state.soundOn =
        typeof data.soundOn === "boolean"
          ? data.soundOn
          : DEFAULT_STATE.soundOn;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (hasUrlParams) {
    ...
    setTimeout(() => {
      // Apply filters
      const filter = `blur(${state.blur}px) brightness(${
  ...
  document.getElementById("url-btn").addEventListener("click", () => {
    ...
    showToast(
      type === "video" ? "✓ Video loaded" : "✓ Image loaded",
      "success"
    );
  });
